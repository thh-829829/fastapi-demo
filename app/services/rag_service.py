...
from app.services.qa_log_service import create_qa_log
...
def normal_rag_qa(question: str, user_id: int, top_n: int = 8, doc_id: int | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if cache_value:
            ...
            result_data = json.loads(cache_value)
            # 缓存命中也写入日志，保证全量可观测
            latency = int((time.time() - start_time) * 1000)
            answer_summary = result_data["answer"][:100]
            create_qa_log(
                user_id=user_id,
                question=question,
        ...
        # 写入成功问答日志
        latency = int((time.time() - start_time) * 1000)
        answer_summary = result["answer"][:100]
