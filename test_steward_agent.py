...
# ========== 管家Agent核心循环 ==========
def steward_agent_run(user_input: str, user_id: int = 1, session_id: str = "steward_default", max_steps: int = 10):
    # 1. 初始化统一上下文管理器
...
6. 范围边界性：只围绕学习任务管理展开，不回答无关问题"""
    ...
    return "执行步数超限，未能完成处理"


# ========== 全场景测试 ==========
if __name__ == "__main__":
    TEST_SESSION = "test_steward_agent_001"
    ...
    print("=" * 60)
    print("【场景1：任务完成汇报 → 自动更新+结构化评价】")
    print("=" * 60)
    user_input1 = "我完成了ID为5的任务"
    print(f"用户输入：{user_input1}")
    result1 = steward_agent_run(user_input1, user_id=1, session_id=TEST_SESSION)
    print(result1)

    print("\n" + "=" * 60)
    print("【场景2：进度查询分析 → 自动统计+风险预警】")
    print("=" * 60)
    user_input2 = "我现在整体学习进度怎么样，有什么风险"
    print(f"用户输入：{user_input2}")
    result2 = steward_agent_run(user_input2, user_id=1, session_id=TEST_SESSION)
    print(result2)

    print("\n" + "=" * 60)
    print("【场景3：学习规划生成 → 目标拆解+每日计划】")
    print("=" * 60)
    user_input3 = "帮我做一个6天的Python数据结构专项学习计划"
    print(f"用户输入：{user_input3}")
    result3 = steward_agent_run(user_input3, user_id=1, session_id=TEST_SESSION)
    print(result3)

    print("\n" + "=" * 60)
    print("【场景4：跨场景多轮 → 基于进度生成针对性计划】")
    print("=" * 60)
    user_input4 = "针对我当前落后的高优先级任务，给我排一个3天的攻坚计划"
    print(f"用户输入：{user_input4}")
    result4 = steward_agent_run(user_input4, user_id=1, session_id=TEST_SESSION)
    print(result4)
